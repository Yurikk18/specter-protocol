...
fn test_full_lifecycle_with_credentials() {
    ...
    let vr = verify::verify_token(&token, &mint.group_public_key(), &mint.pedersen, &mint.credential_issuer.pedersen);
    assert!(vr.all_valid());
    ...
    for i in 0..10 {
        ...
        let vr = verify::verify_token(&result.token, &mint.group_public_key(), &mint.pedersen, &mint.credential_issuer.pedersen);
...
fn test_full_lifecycle_without_credentials() {
    ...
    let vr = verify::verify_token(&current, &mint.group_public_key(), &mint.pedersen, &mint.credential_issuer.pedersen);
    assert!(vr.all_valid());
}
...
fn test_double_spend_detection() {
    let mint = setup_mint(2, 3, 20);
    let token = mint.issue(1000, &[1, 2], None).unwrap();
    let s1 = transfer::transfer(&token).unwrap();
    let s2 = transfer::transfer(&token).unwrap();
    assert_eq!(s1.spent_nullifier, s2.spent_nullifier);
    let mut set = NullifierSet::new();
    assert!(transfer::check_double_spend(&mut set, &s1.spent_nullifier).is_ok());
    assert!(transfer::check_double_spend(&mut set, &s2.spent_nullifier).is_err());
}
...
fn test_recursion_bound() {
    ...
    let mut current = token;
    for _ in 0..10 { current = transfer::transfer(&current).unwrap().token; }
...
fn test_wrong_mint() {
    ...
    let vr = verify::verify_token(&token, &m2.group_public_key(), &m1.pedersen, &m1.credential_issuer.pedersen);
...
fn test_stress_long_chain() {
    ...
    let mut current = token;
    for _ in 0..200 { current = transfer::transfer(&current).unwrap().token; }
...
fn test_multiple_tokens_independent() {
    ...
    for i in 0u64..10 {
        ...
        let mut c = token;
        for _ in 0..3 {
            let r = transfer::transfer(&c).unwrap();
            assert!(transfer::check_double_spend(&mut ns, &r.spent_nullifier).is_ok());
            c = r.token;
        }
        let vr = verify::verify_token(&c, &mint.group_public_key(), &mint.pedersen, &mint.credential_issuer.pedersen);
        assert!(vr.all_valid());
    }
    assert_eq!(ns.len(), 30);
}
