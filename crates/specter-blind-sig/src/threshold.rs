...
/// A threshold keyset generated by a trusted dealer or interactive DKG.
///
/// Contains secret key shares — zeroized on drop.
#[derive(Clone)]
...
pub struct SignerSessionState {
    /// The signer's nonce for this session. Private — exposure enables key recovery.
    nonce: Scalar,
    /// The signer's nonce commitment R_i = nonce * G.
    pub nonce_commitment: RistrettoPoint,
}

impl Drop for ThresholdKeyset {
    fn drop(&mut self) {
        use zeroize::Zeroize;
...
pub fn dealer_keygen(threshold: usize, total: usize) -> ThresholdKeyset {
    let mut group_secret = random_scalar();
    ...
    // Zeroize the full group secret — only shares should persist
    use zeroize::Zeroize;
